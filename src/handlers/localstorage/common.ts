/*global storage*/
import { legacy } from '#/lib/storage/legacy'
import { logger, AutomateError } from '#/logger'
...
export const saveLocal = (key = '', data = {}) => {
  try {
    ...
    logger.error(new AutomateError('Legacy Storage: saveLocal error'))
  }
}
...
export const deprecatedSaveLocal = async (
  ...
  try {
    ...
    logger.error(new AutomateError('Storage: deprecatedSaveLocal error'))
  }
}
  ...
  try {
    ...
    switch (error.name) {
      ...
      case 'ExpiredError':
        break
      default:
        logger.error(new AutomateError('Storage: deprecatedGetLocal error'))
    }
...
export const deprecatedRemoveLocal = (key = '') => {
  try {
    ...
    logger.error(new AutomateError('Storage: deprecatedRemoveLocal error'))
  }
}
