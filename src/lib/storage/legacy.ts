...
import { Legacy } from '#/lib/storage/schema'
  ...
  async get<Key extends keyof Schema>(
    ...
    if (!res) {
      try {
        ...
        logger.error(
          new AutomateError(`Storage: error migrating legacy data`),
          {
            key,
          },
        )
...
export const queryStorage = new LegacyStorage<[], Legacy>({
  id: 'rainbow.react-query',
})
