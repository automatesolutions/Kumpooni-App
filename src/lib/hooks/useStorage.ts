...
export function useStorage<T>(key: string, options?: UseStorageOptions<T>) {
  ...
  const validateValue = useCallback(
    (_value?: T) => {
      if (!validationValues?.length) return _value

      const isValid = validationValues?.includes(_value)
      if (isValid) return _value

      console.warn(
        `Invalid value for ${key}: ${_value}, falling back to previous value: ${value}`,
      )
  ...
  const setValue = useCallback(
    (_value?: T) => {
      ...
      mmkvStorage.set(key, JSON.stringify(validatedValue))
  ...
  useEffect(() => {
    ...
    try {
      if (!item) return setValue(defaultValue) // set default value if no item found and update state

      const parsedItem = JSON.parse(item!)

      if (parsedItem) _setValue(parsedItem) // update state
    } catch (e) {
      console.error(
        `Failed to parse value for ${key}: ${item}, setting to default value: ${defaultValue}`,
      )
